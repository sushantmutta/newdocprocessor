...
class TestPrescriptionSchema:
    ...
    def test_invalid_dosage_format(self):
        """Test that invalid dosage format is accepted at schema level.

        Design Note: Dosage validation is now permissive at schema level.
        The validator agent generates flags for data quality issues.
        This allows data to flow through the pipeline for human review.
        """
        data = {
        ...
        # Schema should accept any dosage format (validation done by agent)
        prescription = PrescriptionSchema(**data)
        assert prescription.medications[0].dosage == "500 liters"
        # Validation flags would be generated by the validator agent
