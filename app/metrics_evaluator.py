...
class MetricsEvaluator:
    ...
    def _load_data(self):
        ...
        if os.path.exists(self.metrics_csv_path):
            try:
                ...
                # Clean percentage strings for all percentage columns
                percentage_cols = ['extraction_completeness', 'validation_accuracy',
                                   'redaction_coverage', 'pii_recall', 'pii_precision']
                for col in percentage_cols:
                    if col in self.df.columns:
                        # Convert to string, strip %, replace empty with 0, convert to float
    ...
    def calculate_workflow_success(self) -> Dict[str, float]:
        ...
        success_count = self.df['pipeline_success'].sum()
    ...
    def generate_llm_insights(self, metrics_data: Dict, provider: str = "groq") -> Dict:
        ...
        try:
            ...
            prompt = f"""You are an AI Performance Analyst for a Medical Document Intelligence Platform. Analyze the following system metrics and provide actionable insights.

SYSTEM METRICS:
==============
Documents Processed: {summary.get('total_documents_processed', 0)}
Overall Compliance: {summary.get('overall_compliance', 'N/A')}

1. EXTRACTION ACCURACY (Target: ≥ 90%)
   - Overall Accuracy: {extraction.get('overall_extraction_accuracy', 0)}%
   - Extraction Completeness: {extraction.get('avg_extraction_completeness', 0)}%
   - Validation Accuracy: {extraction.get('avg_validation_accuracy', 0)}%
   - Status: {'✅ MEETS TARGET' if extraction.get('meets_target') else '❌ BELOW TARGET'}

2. PII REDACTION (Recall Target: ≥ 95%, Precision Target: ≥ 90%)
   - PII Recall: {pii.get('avg_pii_recall', 0)}%
   - PII Precision: {pii.get('avg_pii_precision', 0)}%
   - Recall Status: {'✅ MEETS TARGET' if pii.get('recall_meets_target') else '❌ BELOW TARGET'}
   - Precision Status: {'✅ MEETS TARGET' if pii.get('precision_meets_target') else '❌ BELOW TARGET'}

3. WORKFLOW SUCCESS (Target: ≥ 90%)
   - Success Rate: {workflow.get('success_rate', 0)}%
   - Successful: {workflow.get('success_count', 0)} / {workflow.get('total_count', 0)}
   - Status: {'✅ MEETS TARGET' if workflow.get('meets_target') else '❌ BELOW TARGET'}

4. LATENCY (P95 Target: ≤ 4000ms)
   - Status: {latency.get('note', 'Not tracked')}
...
OUTPUT FORMAT: Valid JSON only, no markdown, no code blocks, no explanations outside JSON."""
            ...
            messages = [
                SystemMessage(
                    content="You are an expert AI Performance Analyst specializing in healthcare document processing systems."),
...
def get_metrics_summary(metrics_csv_path: str = None, include_llm_insights: bool = True, llm_provider: str = "groq") -> Dict:
    ...
    if metrics_csv_path is None:
        ...
        metrics_csv_path = os.path.join(
            base_dir, "reports", "metrics_report.csv")
